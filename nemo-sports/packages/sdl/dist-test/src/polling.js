...
function profileFor(status, opts = {}) {
    let p = BASE[status] ?? BASE.scheduled;
    const now = opts.now ?? Date.now();
    if (status === "scheduled" && opts.kickoffAt) {
        const toKickoff = +new Date(opts.kickoffAt) - now;
        if (toKickoff <= 15 * 60_000)
            p = { intervalSeconds: 30, realtime: true, reason: "kickoff within 15 min — lineups & first whistle" };
        else if (toKickoff <= 60 * 60_000)
            p = { intervalSeconds: 120, realtime: false, reason: "kickoff within an hour" };
    }
    // a freshly finished match is re-checked for a while (late corrections, VAR)
    if (status === "finished")
        p = { intervalSeconds: 900, realtime: false, reason: "settling window after full time" };
