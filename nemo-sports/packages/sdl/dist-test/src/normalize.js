...
exports.nowIso = exports.roundMinutes = void 0;
exports.normalizeName = normalizeName;
exports.compactKey = compactKey;
...
function requestKey(provider, endpoint, params) {
...
const roundMinutes = (v, step = 5) => Math.round(v / step) * step;
...
function similarity(aRaw, bRaw) {
    ...
    if (a === b)
        return 1;
    if (compactKey(aRaw) === compactKey(bRaw))
        return 0.97;
 ...
 */
function rankCandidates(query, pool, threshold = 0.72) {
    return pool
        .map((c) => {
...
function seededId(seed) {
    ...
    const hex32 = (v) => (Number.parseInt(fingerprint(v), 36) >>> 0).toString(16).padStart(8, "0");
